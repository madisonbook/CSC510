...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    ...
    try:
        ...
        if user:
            print(f"[auth] found user: _id={user.get('_id')} email={user.get('email')} verified={user.get('verified')}")
        else:
...
async def get_optional_current_user(credentials: HTTPAuthorizationCredentials = Depends(security_optional)) -> Optional[dict]:
    ...
    try:
        token = credentials.credentials
        user = await db.users.find_one({"email": token})
        # If user exists but not verified, still return None (caller can enforce later)
